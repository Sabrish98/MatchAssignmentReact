...
const lenght = imagesList.length

const randomImgCreate = Math.random() * lenght - 1

const ceilImg = Math.ceil(randomImgCreate)

class Home extends Component {
  state = {
    activetabId: tabsList[0].tabId,
    initialThumnailList: imagesList,
    score: 0,
    seconds: 60,
    randomImg: ceilImg,
    final: 0,
  ...
  timer = () => {
    ...
    if (seconds === 0) {
      ...
      this.setState({final: score, score: 0})
    } else {
  ...
  getTabItemList = () => {
    const {initialThumnailList, activetabId} = this.state

    const items = initialThumnailList.filter(
  ...
  onReset = () => {
    ...
    this.setState({score: 0, seconds: 60})
  }

  matching = id => {
    const {randomImg, score, initialThumnailList} = this.state

    const newNumber = Math.random() * lenght - 1

    const ceilImgNumber = Math.ceil(newNumber)

    if (initialThumnailList[randomImg].id === id) {
      this.setState(preState => ({
        score: preState.score + 1,
        randomImg: ceilImgNumber,
      }))
    }
  }
  ...
  render() {
    ...
    const getthumbById = this.getTabItemList()
    ...
    return (
      <div className="bg-container">
        ...
        <ul>
          {seconds === 0 && <GameOver result={final} onReset={this.onReset} />}
        </ul>
        {seconds !== 0 && (
          <div className="Images">
            <img
              alt="match"
              className="Image"
              key={imagesList[randomImg].imageUrl}
              src={imagesList[randomImg].imageUrl}
            />
          </div>
        )}
